...
for i = 1:length(l)
    [lower_bound, upper_bound, num_iter] = goldenSection(f1, a, b, l(i));
    iterations_f1(i) = num_iter;
    lower_bounds_f1(i) = lower_bound;
    upper_bounds_f1(i) = upper_bound;
    
    [lower_bound, upper_bound, num_iter] = goldenSection(f2, a, b, l(i));
    iterations_f2(i) = num_iter;
    lower_bounds_f2(i) = lower_bound;
    upper_bounds_f2(i) = upper_bound;
    
    [lower_bound, upper_bound, num_iter] = goldenSection(f3, a, b, l(i));
...
% Plot the results
figure;
plot(l, iterations_f1 + 1, '-o', 'DisplayName', 'f1');
hold on;
plot(l, iterations_f2 + 1, '-x', 'DisplayName', 'f2');
plot(l, iterations_f3 + 1, '-s', 'DisplayName', 'f3');
hold off;
